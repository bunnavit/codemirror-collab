...
export class TextNode extends Text {
  ...
  // children is array of TextLeafs/TextNodes
  static from(
    children,
    // length = number of TextLeafs/TextNodes
    length = children.reduce((l, ch) => l + ch.length + 1, -1)
  ) {
    // lines = total number of lines in array of TextLeafs/TextNodes
    let lines = 0;
    for (let ch of children) lines += ch.lines;
    // if lines < 32, flatten and return single TextLeaf
    ...
    let chunked = [],
      ...
      currentLen = -1,
      currentChunk = [];
    // Add TextLeaf/TextNode
    function add(child) {
      let last;
      // If more than maxChunk, convert children of TextNode into TextNodes
      if (child.lines > maxChunk && child instanceof TextNode) {
        ...
        // if isTextLeaf, is currentLines, last currentChunk is a text leaf and child+last lines less than 32, concat remainder to current chunk
...
export class ChangeSet {
  ...
  static fromJSON(json) {
    ...
    for (let i = 0; i < json.length; i++) {
      ...
      // unaffected sections
      // part: len that is unaffected, -1: not affected
      if (typeof part == "number") {
        ...
        throw new RangeError("Invalid JSON representation of ChangeSet");
        // deletion
        // part[0]: len to be deleted, 0: deletion
      } else if (part.length == 1) {
        sections.push(part[0], 0);
      } else {
        // replacement
        // part[0]: entire len that was replaced/inserted, inserted[i].length: len to be inserted
    ...
    case "pushUpdates": {
      if (data.version !== updates.length) {
        ...
        for (var _i = 0, _a = data.updates; _i < _a.length; _i++) {
          ...
          console.log("changesBefore", update.changes);

          var changes = ChangeSet.fromJSON(update.changes);

          console.log("changesAfter", changes);
